...
export function useExamActions(questionBank: ExamQuestion[]) {
  ...
  const launchExam = useCallback(() => {
    ...
    dispatch(startExam({
      questions: q, timeLimit: EXAM_TIME_LIMIT,
    }));
  ...
  const goTo = useCallback(
    (i: number) => dispatch(goToQuestion(i)),
    [dispatch],
  );

  return {
    launchExam, answer, submit, retake,
    backToStart, review, next, prev, goTo,
  };
}
