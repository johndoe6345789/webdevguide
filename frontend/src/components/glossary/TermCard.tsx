...
export default function TermCard({ term, definition, category, relatedTerms, example }: GlossaryTerm) {
  return (
    <Card sx={{ height: '100%' }}>
      <CardContent>
        ...
        {example && (
          <Paper variant="outlined" sx={{ p: 1.5, mb: 2, bgcolor: 'action.hover' }}>
            <Typography variant="caption" fontWeight={600} color="text.secondary">Example:</Typography>
            <Typography variant="body2" sx={{ mt: 0.5, fontFamily: 'monospace' }}>{example}</Typography>
          </Paper>
        )}
        {relatedTerms.length > 0 && (
          <Box>
            <Typography variant="caption" color="text.secondary" fontWeight={600}>Related: </Typography>
            {relatedTerms.map((rt) => (
